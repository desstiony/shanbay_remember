...
async function openai_worlds(worlds) {
    ...
    let messages = [{ role: "system", content: "You are helpful English Teaching Assistant" }];
    try {
        messages.push({ role: "user", content: "请根据下边的单词，使用英文编写一篇简短的故事，并分段逐句添加中文释意，注意排版清晰可读。" + worlds });
    ...
    init() {
        Func.loop(MIN_LOOP - 1, (idx) => {
            this.status[(idx + 1) & 3] = Num.xor(
                this.status[(idx + 1) & 3],
                idx + 1 + Num.mul(1812433253, Num.xor(this.status[idx & 3], Num.shiftRight(this.status[idx & 3], 30)))
            );
        });

        if ((this.status[0] & BAY_MASK) === 0 && this.status[1] === 0 && this.status[2] === 0 && this.status[3] === 0) {
            this.status[0] = 66;
            this.status[1] = 65;
            this.status[2] = 89;
            this.status[3] = 83;
